export type GeneratedLpcRuntimeSlot =
  | 'weapon'
  | 'helmet'
  | 'armor'
  | 'boots';
export type GeneratedLpcRuntimeVisualSlot =
  | 'weapon'
  | 'helmet'
  | 'chest'
  | 'boots';
...
export type GeneratedLpcRuntimeAsset =
  | GeneratedLpcRuntimeSimpleAsset
  | GeneratedLpcRuntimeSplitAsset;
...
const RUNTIME_MAP_URLS = Object.freeze({
  weapon:
    '/assets/dungeon-overlay/lpc-v1/runtime/generated-weapon-map.json',
  helmet:
    '/assets/dungeon-overlay/lpc-v1/runtime/generated-helmet-map.json',
  armor:
    '/assets/dungeon-overlay/lpc-v1/runtime/generated-armor-map.json',
  boots:
    '/assets/dungeon-overlay/lpc-v1/runtime/generated-boots-map.json',
} satisfies Record<GeneratedLpcRuntimeSlot, string>);

const runtimeMaps = new Map<
  GeneratedLpcRuntimeSlot,
  ReadonlyMap<string, GeneratedLpcRuntimeItem> | null
>();
...
const isSplitAsset = (
  value: unknown,
): value is GeneratedLpcRuntimeSplitAsset =>
  Boolean(
    value &&
      typeof value === 'object' &&
...
const isAsset = (value: unknown): value is GeneratedLpcRuntimeAsset =>
  typeof value === 'string' || isSplitAsset(value);

const isRuntimeItem = (
  value: unknown,
  slot: GeneratedLpcRuntimeSlot,
): value is GeneratedLpcRuntimeItem => {
...
const warnMapFailure = (
  ...
  console.warn(
    `[LPC V1] Failed to load generated ${slot} runtime map; that visual slot is disabled.`,
    error,
  );
};
  ...
  const promise = fetch(RUNTIME_MAP_URLS[slot], {
    ...
    .then(async (response) => {
      ...
      const invalidIndex = parsed.findIndex(
        (item) => !isRuntimeItem(item, slot),
      );
      if (invalidIndex !== -1) {
        throw new TypeError(
          `Generated ${slot} runtime map has an invalid item at index ${invalidIndex}.`,
        );
      }
      const map = new Map(
        parsed.map((item) => [item.internalItemId, item] as const),
      );
...
export const generatedLpcRuntimeVisualSlot = (
  slot: GeneratedLpcRuntimeSlot,
): GeneratedLpcRuntimeVisualSlot => (slot === 'armor' ? 'chest' : slot);

export const generatedLpcRuntimeSlotFor = (
  visualSlot: string,
  itemId: string,
): GeneratedLpcRuntimeSlot | null => {
  ...
  const expectedSlot =
    visualSlot === 'chest'
      ...
      : visualSlot === 'weapon' ||
        ...
        ? visualSlot
        : null;
  return expectedSlot && normalizedId.startsWith(`lpc-${expectedSlot}-`)
    ? expectedSlot
    : null;
};
